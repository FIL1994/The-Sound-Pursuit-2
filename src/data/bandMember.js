...
export function doPractice(val, base) {
  let practiceIncrease =
    (base + _.random(5.1, 5.5)) / 20 * (1 + (90 - val) / 100);
...
export default level => {
  ...
  for (let i = 0; i < 10; i++) {
    switch (
      CHANCE.weighted(
        ...
        [
          10 - baseSkills.songwriting,
          10 - baseSkills.musicianship,
          10 - baseSkills.live,
          10 - baseSkills.studio
        ]
      )
    ) {
      case "songwriting":
  ...
  if (level > 3) {
    ...
    for (let i = 0; i < timesToPractice; i++) {
      skills.songwriting = doPractice(
        skills.songwriting,
        baseSkills.songwriting
      );
      skills.musicianship = doPractice(
        skills.songwriting,
        baseSkills.musicianship
      );
