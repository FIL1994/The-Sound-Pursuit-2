...
import {
  fiveYearScoreboardID,
  tenYearScoreboardID,
  bestSellingAlbumsScoreboardID,
  bestSellingSinglesScoreboardID,
  totalAlbumSalesScoreboardID,
  totalSingleSalesScoreboardID,
  totalFansScoreboardID
} from "../config/keys";
...
export function saveBand(band) {
  return dispatch => {
    return localForage.setItem(DATA_BAND, band).then((val, error) => {
      if (error) {
...
export function getBand() {
  return dispatch => {
    return localForage.getItem(DATA_BAND).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_BAND, error }));
      } else {
...
export function getSongs() {
  return dispatch => {
    return localForage.getItem(DATA_SONGS).then((val, error) => {
      if (error) {
...
export function saveSongs(songs) {
  return dispatch => {
    return localForage.setItem(DATA_SONGS, songs).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_SONG, error }));
      } else {
...
export function writeSong(song) {
  return dispatch => {
    return localForage.getItem(DATA_SONGS).then((songs, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_SONG, error }));
      } else {
...
export function getWeek() {
  return dispatch => {
    return localForage.getItem(DATA_WEEK).then((val, error) => {
      if (error) {
...
export function saveWeek(week) {
  return dispatch => {
    return localForage.setItem(DATA_WEEK, week).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_WEEK, error }));
      } else {
...
export function nextWeek(weeks, tourDetails = {}) {
  return dispatch => {
    return localForage.getItem(DATA_WEEK).then((week, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_WEEK, error }));
      } else {
        let years5 = false,
          years10 = false,
          tourResults = { newFans: 0, newCash: 0 };
        ...
        Promise.all([
          ...
          for (let i = 0; i < weeks; i++) {
            ...
            let tourFans = 0,
              tourCash = 0;
            if (onTour) {
              const { newCashFromTour, newFansFromTour } = calcTourResults({
                fans,
                band,
                tourDetails
              });
          ...
          // unlock medals
          Promise.all(
            unlocks.map(unlockFunction => new Promise(unlockFunction))
          );
          localForage
            .setItem(DATA_WEEK, week)
            .then(() =>
              dispatch(
                sendReturn({ type: GET_WEEK, payload: week, years5, years10 })
              )
            );
        });
      }
  ...
  function calcSales({ albums, singles, week, fans, charts, dispatch }) {
    ...
    singles.forEach(({ released, quality, salesLastWeek }, index) => {
      ...
      if (age < SINGLE_SALES_LAST) {
        let sales = Math.ceil(
          (fans *
            (quality / 125) *
            ((SINGLE_SALES_LAST - age) / (SINGLE_SALES_LAST - 1)) +
            1) *
        ...
        // calculate cash
        newCash += sales * 0.12; // $0.12 for each single sold
    ...
    setTimeout(() => postScore(totalSingleSales, totalSingleSalesScoreboardID));
    ...
    albums.forEach(({ released, quality, salesLastWeek }, index) => {
      ...
      if (age < ALBUM_SALES_LAST) {
        let sales = Math.ceil(
          (fans *
            (quality / 180) *
            ((ALBUM_SALES_LAST - age) / (ALBUM_SALES_LAST - 1)) +
            1) *
            _.random(0.135, 0.193)
        );
        ...
        setTimeout(() => postScore(albumSales, bestSellingAlbumsScoreboardID));
      } else if (salesLastWeek !== 0) {
    ...
    setTimeout(() => postScore(totalAlbumSales, totalAlbumSalesScoreboardID));
...
export function getFans() {
  return dispatch => {
    return localForage.getItem(DATA_FANS).then((val, error) => {
      if (error) {
...
export function saveFans(fans) {
  return dispatch => {
    return localForage.setItem(DATA_FANS, fans).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_FANS, error }));
      } else {
        fans = Number(fans);
        setTimeout(() => postScore(fans, totalFansScoreboardID));
...
export function getCash() {
  return dispatch => {
    return localForage.getItem(DATA_CASH).then((val, error) => {
      if (error) {
...
export function saveCash(cash) {
  return dispatch => {
    ...
    return localForage.setItem(DATA_CASH, cash).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_CASH, error }));
      } else {
...
export function addCash(amount) {
  return dispatch => {
    return localForage.getItem(DATA_CASH).then((cash, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_CASH, error }));
      } else {
        // if val is NaN set val to default cash
...
export function getCharts() {
  return dispatch => {
    return localForage.getItem(DATA_CHARTS).then((charts, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_CHARTS, error }));
      } else {
        if (_.isEmpty(charts)) {
          return localForage.getItem(DATA_WEEK).then(week => {
  ...
  return dispatch => {
    if (_.isEmpty(charts)) {
      dispatch(
        sendReturn({
          type: ERROR_CHARTS,
          error: new Error("SAVE CHARTS - charts was empty")
        })
      );
    } else {
...
export function getSingles() {
  return dispatch => {
    return localForage.getItem(DATA_SINGLES).then((singles, error) => {
      if (error) {
...
export function saveSingles(singles) {
  return dispatch => {
    return localForage.setItem(DATA_SINGLES, singles).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_SINGLES, error }));
      } else {
...
export function getAlbums() {
  return dispatch => {
    return localForage.getItem(DATA_ALBUMS).then((albums, error) => {
      if (error) {
...
export function saveAlbums(albums) {
  return dispatch => {
    return localForage.setItem(DATA_ALBUMS, albums).then((val, error) => {
      if (error) {
        dispatch(sendReturn({ type: ERROR_ALBUMS, error }));
      } else {
...
function calculateScore({ years, albums, singles, fans }) {
  ...
  setTimeout(() => postScore(score, scoreboardID));
...
export function getRandomImage() {
  return dispatch =>
    fetchRandomImage().then(
      ({ url }) => {
        dispatch(sendReturn({ type: GET_IMAGE, payload: url }));
        return url;
      },
      error => {
        console.log(error);
